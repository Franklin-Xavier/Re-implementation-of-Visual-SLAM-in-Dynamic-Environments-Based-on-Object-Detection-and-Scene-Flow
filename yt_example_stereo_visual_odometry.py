...
class VisualOdometry():
    def __init__(self, data_dir):
        ...
        self.gt_poses = self._load_poses(data_dir + '/true_T.txt')
        self.images_l = self._load_images(data_dir + '/Left_Images')
        self.images_r = self._load_images(data_dir + '/Right_Images')
    ...
    def _load_calib(filepath):
        ...
        camera_param = ReadCameraParam(filepath)
        P_l = camera_param['left_projection']
        K_l = P_l[0:3, 0:3]
        P_r = camera_param['right_projection']
        K_r = P_r[0:3, 0:3]
...
def main():
    ...
    for i, gt_pose in enumerate(tqdm(vo.gt_poses, unit="poses")):
        ...
        gt_path.append(gt_pose[2, 3])
        estimated_path.append(cur_pose[2, 3])
