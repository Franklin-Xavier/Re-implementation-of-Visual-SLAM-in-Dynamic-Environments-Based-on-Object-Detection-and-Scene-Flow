...
if __name__ == "__main__":
    ...
    for ind in range(1, len(left_images) - 1):
        ...
        ############################## Visulizaing Feature Matching between Time Frames #########################
        ##############################         (Points inside of bounding boxes)        #########################
        ...
        #                                               paired_poten_dy_fpts_cur_2d_pre_2d[:, 2:],
        #                                               paired_poten_dy_fpts_cur_2d_pre_2d[:, :2],
        #                                               pic_title='')
        # plt.pause(20)
        # plt.close()
        ...
        ############################## Visulizaing Feature Matching between Time Frames #########################
        ##############################         (Points outside of bounding boxes)       ########################
        ...
        while dynamic_included < 2:
            ...
            if dynamic_included == 0:
                ...
                # Visualize Candidate Feature Points before KL

                # radius = 2
                # color_1 = (0, 255, 0)
                # thickness = 2

                # img_1_copy = left_image.copy()

                # for i, (x, y) in enumerate(paired_static_features_current_2d_previous_3d[:, :2]):
                #     cv2.circle(img_1_copy, (int(x), int(y)), radius, color_1, thickness)
                
                # fig = plt.figure(figsize=(10, 5))

                # plt.title('Used feature points')
            ...
            if dynamic_included == 1:
                ...
                if len(more_static_points)!=0:

                    for i, (x, y) in enumerate(paired_static_features_current_2d_previous_3d[:-len(more_static_points), :2]):
                        cv2.circle(img_1_copy, (int(x), int(y)), radius, color_1, thickness)

                    for i, (x, y) in enumerate(paired_static_features_current_2d_previous_3d[-len(more_static_points):, :2]):
                        cv2.circle(img_1_copy, (int(x), int(y)), radius, color_2, thickness)       
